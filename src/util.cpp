...
const char * const BITCOIN_CONF_FILENAME = "stargate.conf";
const char * const BITCOIN_PID_FILENAME = "stargated.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stargate" is a composite category enabling all stargate-related debug output
            if(ptrCategory->count(string("stargate"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\stargateCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\stargateCore
    // Mac: ~/Library/Application Support/stargateCore
    // Unix: ~/.stargatecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "stargateCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/stargateCore";
#else
    // Unix
    return pathRet / ".stargatecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stargate.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stargate.conf
