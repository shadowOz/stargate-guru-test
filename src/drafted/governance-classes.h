 ...
 *
 *   // network
 *   CstargateNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.stargate.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CstargateNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CstargateNetwork(UniValue objIn)
    {
...
// // can be under: stargateNetwork
// //   -- signature requirements : Key1(User)
// class CstargateNetworkVariable : public CGovernanceObject
// {
// private:
