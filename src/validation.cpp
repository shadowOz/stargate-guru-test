...
#if defined(NDEBUG)
# error "Iluminum Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Iluminum Signed Message:\n";
...
{
    CAmount ret = (blockValue/100) *70; // start at 70%
    ...
    if(nHeight > nMNPIBlock)                   ret = (blockValue/100) *70; // 70%

    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1))  ret = (blockValue/100) *75; // 75%
    
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2))  ret = (blockValue/100) *80; // 80%
...
void ThreadScriptCheck() {
    RenameThread("iluminum-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // ILUM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ILUM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ILUM): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ILUM : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ILUM): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ILUM): spork is off, skipping transaction locking checks\n");
    }
